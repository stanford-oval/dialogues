...
$(experiment)/$(source)/input: $(dataset_folder)/$(source)/$(experiment)
	mkdir -p $@
	...
	for f in $(all_names) ; do \
		if $(clean_input_quotes) ; then \
			python3 ../spl/scripts/text_edit.py --experiment $(experiment) --no_lower_case --remove_qpis --e2e --experiment $(experiment) --num_columns $(num_columns) --experiment $(experiment) --input_file $</$$f.tsv --output_file $@/$$f.tsv ; \
		else \
			cp $</$$f.tsv $@/$$f.tsv ; \
		fi ; \
	done
...
$(experiment)/$(source)/input-nmt: $(experiment)/$(source)/input-qpis
	...
	for f in $(all_names) ; do \
		python3 ../spl/scripts/text_edit.py --experiment $(experiment) --no_lower_case --prepare_for_marian --e2e --replace_ids  --num_columns $(num_columns) --input_file $</$$f.tsv --output_file $@/$$f.tmp.tsv ; \
...
# make -B all_names=valid experiment=risawoz_zh source=v10 src_lang=zh  process_data