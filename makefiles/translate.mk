...
$(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/refined-qpis: $(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/translated-qpis
	...
	# refine sentence
	for f in $(all_names) ; do \
		python3 ./scripts/text_edit.py --no_lower_case --e2e --refine_sentence --post_process_translation --experiment $(experiment) --param_language $(src_lang) --num_columns $(num_columns) --input_file $@/$$f.tmp.tsv --output_file $@/$$f.tsv ; \
	done
...
$(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/cleaned-qpis: $(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/refined-qpis
	...
	for f in $(all_names) ; do \
		if $(is_user_acts) ; then \
			...
			python3 ./scripts/text_edit.py --experiment $(experiment) --no_lower_case --e2e --insert_space_quotes --num_columns $(num_columns) --input_file $</$$f.tsv --output_file $@/$$f.tsv ; \
		fi ; \
	done
...
$(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/cleaned: $(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/cleaned-qpis
	...
	for f in $(all_names) ; do \
		python3 ./scripts/text_edit.py --experiment $(experiment) --no_lower_case --e2e --remove_qpis `if $(is_user_acts) ; then echo "--is_user_acts" ; fi` --num_columns $(num_columns) --input_file $</$$f.tsv  --output_file $@/$$f.tsv ; \
	done
...
$(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/final: $(experiment)/$(source)/$(nmt_model)/$(tgt_lang)/augmented
	mkdir -p $@
	# remove cjk spaces
	for f in $(all_names) ; do python3 ./scripts/text_edit.py --no_lower_case --e2e --fix_spaces_cjk --experiment $(experiment) --param_language $(tgt_lang) --num_columns $(num_columns) --input_file $</$$f.tsv  --output_file $@/$$f.tsv  ; done
